...
export class TripRiderEntity {
    ...
    @Column({ type: 'numeric', precision: 10, scale: 2, nullable: true, name: 'fare_share' })
    ...
    // Relations
    @ManyToOne(() => TripEntity, (trip) => trip.tripRiders, { onDelete: 'CASCADE' })
    @JoinColumn({ name: 'trip_id' })
    trip!: TripEntity;

    @ManyToOne(() => UserEntity, (user) => user.tripRiders, { onDelete: 'CASCADE' })
